...
function buildPerRootInsights(items: MappedDirectoryEntry[]): RootInsight[] {
  ...
  for (const item of items) {
    ...
    if (item.target_exists) {
      current.presentOnDisk += 1;
    }

    const arrState = String(item.arr_state ?? "");
    if (arrState && arrState !== "not_managed") {
      current.hasArrState = true;
    }
    if (arrState === "ok" || arrState === "title_path_mismatch") {
      current.arrOk += 1;
    } else if (arrState === "missing_in_arr") {
      current.notInArr += 1;
    } else if (arrState === "not_managed") {
      current.notManaged += 1;
    }
    ...
    <Card withBorder>
      ...
      <Stack gap="md">
        {rootInsights.map((insight) => {
          ...
          if (insight.hasArrState) {
            // Full Arr breakdown: matched in Arr / not yet in Arr / issues
            ...
            const notManagedPct = (insight.notManaged / total) * 100;
            ...
            return (
              <Stack key={insight.shadowRoot} gap={4}>
                ...
                <Progress.Root size="lg">
                  ...
                  {notManagedPct > 0 && (
                    <Progress.Section value={notManagedPct} color="gray" />
                  )}
                </Progress.Root>
                <Group gap="md">
                  <Text size="xs" c="dimmed">
                    <Text span c="teal" fw={600}>●</Text> {insight.arrOk} matched
                  </Text>
                  {insight.notInArr > 0 && (
                    <Text size="xs" c="dimmed">
                      <Text span c="yellow" fw={600}>●</Text> {insight.notInArr} unmatched
                    </Text>
                  )}
                  {issueCount > 0 && (
                    <Text size="xs" c="dimmed">
                  ...
                  {insight.notManaged > 0 && (
                    <Text size="xs" c="dimmed">
                      <Text span c="gray" fw={600}>●</Text> {insight.notManaged} not managed
                    </Text>
                  )}
                </Group>
              </Stack>
          ...
          // No Arr data: show disk presence
          const presentPct = (insight.presentOnDisk / total) * 100;
          const missingPct = (insight.missingTarget / total) * 100;
          ...
          return (
            <Stack key={insight.shadowRoot} gap={4}>
              ...
              <Progress.Root size="lg">
                <Progress.Section value={presentPct} color="blue" />
                {missingPct > 0 && (
                  <Progress.Section value={missingPct} color="red" />
                )}
              </Progress.Root>
              <Group gap="md">
                <Text size="xs" c="dimmed">
                  <Text span c="blue" fw={600}>●</Text> {insight.presentOnDisk} on disk
                </Text>
                {insight.missingTarget > 0 && (
                  <Text size="xs" c="dimmed">
                    <Text span c="red" fw={600}>●</Text> {insight.missingTarget} missing
                  </Text>
                )}

...
class AutoAddProfileResolver:
    ...
    def resolve(self, folder: Path) -> int | None:
    ...
    def _get_quality_definition_rank_map(self) -> dict[int, int]:
        ...
        try:
            definitions = self._get_radarr_client().get_quality_definitions()
        except Exception as exc:
            self.log.warning(
    ...
    def _detect_custom_format_ids(self, folder: Path) -> set[int]:
        ...
        for candidate in parse_candidates_for_folder(folder, VIDEO_EXTENSIONS):
            if not candidate.strip():
                continue
            try:
                parse_result = self._get_radarr_client().parse_title(candidate)
            except Exception as exc:
    ...
    def _get_auto_add_profiles(self) -> list[dict]:
        ...
        profiles = self._get_radarr_client().get_quality_profiles()
